...
The observability Docker Compose file includes a `demo` profile for this layout. It starts a locked demo controller and static web bundle against the same PostgreSQL database and observability services:

```bash
docker compose -f deployments/docker/compose.observability.yaml --profile demo up -d
```
...
The backend runtime settings are the source of truth. The Vite flags only control the static frontend bundle built for that deployment.
