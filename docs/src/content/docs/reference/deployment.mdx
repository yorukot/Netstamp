...
:::note The public docs site, app runtime, and Grafana runtime are separate deploy targets. GitHub Pages serves the Astro static build for `netstamp.dev`; the bundled nginx image only serves `app.netstamp.dev` for the React app and API. :::
...
The reverse proxy must preserve the `Authorization` header for personal access tokens. Netstamp does not enable CORS for PAT endpoints; use curl, CI, CLI tools, or backend services rather than shipping a token in browser JavaScript.
