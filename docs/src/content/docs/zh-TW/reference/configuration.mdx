...
Release 的 `.env.example` 刻意只保留五個永遠必填的機密值，以及兩個只在 production 使用的設定。若要覆寫任何預設值，請從下列表格將對應變數加入自己的 `.env`。

## 設定的管理位置

基礎設施設定只由環境變數管理，包括資料庫連線、加密與雜湊金鑰、公開來源網址、HTTP 監聽位址、背景工作參數，以及其他行程層級的設定；變更後必須重新啟動控制器。

需要在 Netstamp 執行期間調整的營運設定會由控制器儲存。**管理 → 系統設定** 提供五個彼此獨立的區段：

| 區段   | 執行階段設定                                                    |
| ------ | --------------------------------------------------------------- |
| 存取   | 帳號建立、電子郵件驗證、專案建立，以及使用者憑證變更            |
| SMTP   | 郵件伺服器、寄件者、TLS、逾時時間，以及加密憑證                 |
| OIDC   | 通用 OpenID Connect 提供者與加密的 client secret                |
| Google | Google 提供者、受允許的 hosted domain，以及加密的 client secret |
| GitHub | GitHub 提供者、註冊行為，以及加密的 client secret               |

網頁分析與追蹤同意不是管理介面的系統設定，也不會由執行階段設定 API 回傳。它們會透過下方的 `VITE_NETSTAMP_*` 建置變數編譯進靜態網頁應用程式。
...
| `TIMESCALEDB_IMAGE` | `timescale/timescaledb:2.20.3-pg16` | 資料庫映像檔                             |
...
所有非 local 部署都必須將資料庫密碼與四個應用程式金鑰換成各自獨立的隨機值，且應用程式金鑰至少要有 32 個字元。變更設定加密金鑰可能使已儲存的 SMTP 或外部驗證提供者機密值無法解密。

## HTTP 與公開來源網址

| 變數                       | 預設值  | 用途                                                  |
| -------------------------- | ------- | ----------------------------------------------------- |
| `PUBLIC_BASE_URL`          | empty   | 回呼、連結、OpenAPI 與探測器安裝共用的唯一公開 origin |
...
公開 origin 只能包含通訊協定與主機名稱。非 local 環境必須設定 `PUBLIC_BASE_URL`，production 必須使用 HTTPS。
...
變更雜湊金鑰會使受該金鑰保護的既有憑證失效。帳號建立、電子郵件驗證、專案建立、憑證變更與外部驗證提供者，分別在 **管理 → 系統設定** 的存取、OIDC、Google 與 GitHub 區段中設定。
...
SMTP 在 **管理 → 系統設定** 中管理。SMTP 密碼與外部驗證提供者的 client secret 會使用 `SYSTEM_SETTINGS_ENCRYPTION_KEY` 加密。網頁分析則是建置期設定。
...
這些值會編譯進靜態前端。容器啟動後再變更執行階段環境，不會修改已建置的程式套件。
...
| `VITE_NETSTAMP_GOOGLE_TAG_ID`              | empty                               | Google tag ID；留空即可停用                    |
| `VITE_NETSTAMP_GA_MEASUREMENT_ID`          | empty                               | `VITE_NETSTAMP_GOOGLE_TAG_ID` 的舊版別名       |
| `VITE_NETSTAMP_CLARITY_PROJECT_ID`         | empty                               | Microsoft Clarity project ID；留空即可停用     |
| `VITE_NETSTAMP_META_PIXEL_ID`              | empty                               | Meta Pixel ID；留空即可停用                    |
| `VITE_NETSTAMP_FACEBOOK_PIXEL_ID`          | empty                               | `VITE_NETSTAMP_META_PIXEL_ID` 的舊版別名       |
| `VITE_NETSTAMP_POSTHOG_KEY`                | empty                               | 公開的 PostHog project key；留空即可停用       |
| `VITE_NETSTAMP_POSTHOG_HOST`               | `https://us.i.posthog.com`          | PostHog 資料接收主機                           |
| `VITE_NETSTAMP_PLAUSIBLE_DOMAIN`           | empty                               | 回報給 Plausible 的網域；留空即可停用          |
| `VITE_NETSTAMP_PLAUSIBLE_SCRIPT_URL`       | `https://plausible.io/js/script.js` | Plausible script URL                           |
| `VITE_NETSTAMP_UMAMI_WEBSITE_ID`           | empty                               | Umami website ID；留空即可停用                 |
...
| `VITE_NETSTAMP_TRACKING_CONSENT_COUNTRIES` | 內建地區清單                        | 以逗號或空白分隔的 ISO 3166-1 alpha-2 國家代碼 |

同時設定 Google 或 Meta 的主要名稱與舊版別名時，主要名稱優先。Tracker 識別碼、主機、URL、同意規則及其他所有 `VITE_*` 值都會出現在瀏覽器程式套件中；絕對不要在這些變數中放入私有 API key、client secret、密碼或其他機密值。

同意模式的行為如下：

- `regional`：只對指定國家及無法判斷所在國家的訪客要求同意；
- `always`：所有訪客都必須先同意，才載入已設定的 tracker；
- `immediate`：不顯示同意提示，立即載入已設定的 tracker。

同意國家變數留空時會使用 Netstamp 的內建地區清單。存取政策與示範模式仍會在執行時從控制器載入，但分析及同意設定不會。
