...
變更雜湊金鑰會使受該金鑰保護的既有憑證失效。註冊、電子郵件驗證、專案建立、憑證變更與外部驗證提供者皆在 **Admin → System settings** 中設定。
...
SMTP 與網頁分析設定只在 **Admin → System settings** 中管理。機密值會使用 `SYSTEM_SETTINGS_ENCRYPTION_KEY` 加密。
...
Web 應用程式會在執行時從控制器載入存取政策、示範模式、分析服務與同意設定；這些項目皆在 **管理 → 系統設定** 中管理。
