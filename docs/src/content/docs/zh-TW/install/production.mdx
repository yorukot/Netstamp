...
`PUBLIC_BASE_URL` 是 OAuth/OIDC callback URL、電子郵件與瀏覽器連結、通知、OpenAPI，以及控制器提供的探測器安裝網址共用的唯一公開 origin。它只能包含 HTTPS scheme 與 host，不可包含 path、query、fragment 或帳密資訊。
...
請在部署環境中確認 `PUBLIC_BASE_URL`。公開來源網址與其他基礎設施設定無法從系統管理介面修改。
...
- [ ] `PUBLIC_BASE_URL` 與實際公開的 HTTPS origin 完全相同。
- [ ] 受信任代理範圍沒有超出必要範圍。
- [ ] 帳號建立與電子郵件驗證政策符合預期。
- [ ] SMTP 與每個警示通知目的地都已測試。
