...
若既有安裝仍使用 `BACKEND_BASE_URL` 或 `PUBLIC_WEB_BASE_URL`，升級時必須刪除這兩個欄位，並將共用的 origin 設為 `PUBLIC_BASE_URL`。目前的控制器不接受已移除的舊欄位。
...
請將 dump、Compose 檔案與加密後的 `.env` 備份一起保存。若要解讀已儲存的設定並保留既有憑證，該檔案中的資料庫密碼、加密金鑰與雜湊金鑰都必須能夠復原。

## 升級

編輯 `.env`，指定已測試的 Netstamp 與 TimescaleDB 版本。執行 migration 前，必須先停止所有使用該資料庫的 controller，避免舊版 controller 存取已為新版本完成遷移的 schema。標準 Compose 部署請停止 `netstamp`；若使用 observability stack、demo controller 或額外 replicas，也必須全部停止。

完成備份後，請先更新資料庫 container 與已安裝的 extension，再執行 Netstamp migrations：
...
遷移 container 必須成功結束，任何 controller 才可啟動。Migration、controller 與隨附的 web app 必須使用同一個 Netstamp release；維護時段內不得同時執行新舊 controller。接著驗證健康檢查、登入、一筆結果查詢、一部探測器的心跳，以及一個公開狀態頁。

:::caution 將 container 映像檔切回舊版，不代表資料庫遷移也會自動還原。若 release 明確提供 Goose down migration，請保持所有 controller 停止，先完成 down migration，再啟動舊版映像檔；若沒有明確支援 schema downgrade，則應還原升級前建立的資料庫備份。 :::
