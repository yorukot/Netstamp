...
Fill the five empty secret fields in `deployments/.env` with independent local-development values, then start PostgreSQL:

```bash
docker compose --env-file deployments/.env \
  -f deployments/compose.yaml up -d postgres
```
...
Set `DATABASE_PASSWORD` in `server/.env` to the same value used by `deployments/.env`, then apply migrations:
