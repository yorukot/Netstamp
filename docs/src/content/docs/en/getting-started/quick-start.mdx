...
<ProductScreenshot src={createProjectScreenshot} alt="First-project onboarding form with Home Lab entered as the project name" caption="Create the first project before adding probes and checks." />
...
<ProductScreenshot
	src={probeLocationScreenshot}
	alt="Probe identity form with a name and manual coordinates"
	caption="Give the probe a stable name and a location that identifies its monitoring viewpoint."
/>
...
<ProductScreenshot
	src={probeInstallScreenshot}
	alt="Probe install step with a one-time registration token and install command"
	caption="Run the controller-generated command on the Linux host. The example token shown here is deliberately invalid."
/>

6. Wait for the wizard to show **Heartbeat received**, then select **Finish**.

<ProductScreenshot
	src={probeHeartbeatScreenshot}
	alt="Probe wizard showing that the first heartbeat was received"
	caption="The probe is ready when the controller confirms a signed runtime heartbeat."
/>
...
<ProductScreenshot
	src={createPingCheckScreenshot}
	alt="New Ping check form for the Cloudflare DNS target"
	caption="A Ping check combines its target and schedule with the probes selected below the type-specific settings."
/>
...
<ProductScreenshot
	src={firstResultScreenshot}
	alt="Ping metrics and latency series in Netstamp Insight"
	caption="Select one probe and check in Insight to confirm that the first result reached the controller."
/>
