...
When upgrading an installation that still uses `BACKEND_BASE_URL` or `PUBLIC_WEB_BASE_URL`, remove both keys and set their shared origin as `PUBLIC_BASE_URL`. The removed keys are not accepted by the current controller.
...
Store the dump together with the Compose files and an encrypted copy of `.env`. The database password, encryption key, and hash keys in that file must remain available to interpret stored settings and preserve existing credentials.

## Upgrade

Edit `.env` to select tested Netstamp and TimescaleDB versions. Stop every controller that uses the database before running migrations; this prevents an old controller from accessing a schema that has already been migrated for the new release. For the standard Compose deployment, stop `netstamp`. If you run the observability stack, demo controller, or additional replicas, stop all of those controllers as well.

After creating the backup, update the database container and the installed extension before running Netstamp migrations:
...
The migration container must exit successfully before any controller starts. Use the same Netstamp release for the migration, controller, and bundled web app; do not run old and new controller versions concurrently during this maintenance window. Verify health, sign-in, one result query, one probe heartbeat, and one public status page.

:::caution Do not assume that switching the container image back also reverses database migrations. Keep all controllers stopped while applying a documented Goose down migration, and only then start the old image. Restore the pre-upgrade database backup when the target release does not explicitly support a schema downgrade. :::
