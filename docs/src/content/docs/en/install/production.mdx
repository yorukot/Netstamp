...
`PUBLIC_BASE_URL` is the single public origin used for OAuth/OIDC callback URLs, email and browser links, notifications, OpenAPI, and controller-served probe installer URLs. It must contain only the HTTPS scheme and host, with no path, query, fragment, or credentials.
...
1. configure and test SMTP if needed;
2. decide whether account creation remains enabled;
3. decide whether new accounts must verify email;
4. configure and test each required external authentication provider;
5. add at least one additional system administrator for recovery.

Verify `PUBLIC_BASE_URL` in the deployment environment. The public origin and other infrastructure settings are not editable in Admin.
...
- [ ] Account-creation and email-verification policy are intentional.
