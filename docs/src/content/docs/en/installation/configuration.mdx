...
| `APP_VERSION`                    | `0.1.0`                 | Reported version; Compose maps `NETSTAMP_VERSION` here                |
...
These values are compiled into the frontend. Changing a container's runtime environment does not modify an already-built bundle.
...
Legacy aliases `VITE_NETSTAMP_GA_MEASUREMENT_ID` and `VITE_NETSTAMP_FACEBOOK_PIXEL_ID` remain supported, but the preferred Google and Meta names win when both are set. Every `VITE_*` value is public to browsers; never put a private secret in one.
...
The docs support the same optional trackers and consent controls with the `PUBLIC_NETSTAMP_*` prefix.
