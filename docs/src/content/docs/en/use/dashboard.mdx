...
description: Read the current project's probe health, active checks, network map, and recent probe registry at a glance.
icon: activity
navSection: use
navOrder: 0
head:
...
The network map can only place probes with known coordinates, so its marker count may be lower than the total fleet count. A missing map marker does not mean that a probe is offline.

<ProductScreenshot
	src={probeMapScreenshot}
	alt="Netstamp Overview showing probe health cards and probes positioned on the network map"
	caption="Overview combines fleet health, active checks, and positioned probes for the selected project."
/>
