...
Open [`/openapi/`](/openapi/) from the docs nav. The page loads the generated contract and groups operations by the tags defined in TypeSpec.

Use it to verify request bodies, response schemas, and route paths before writing additional guides.

Backend non-production environments also expose Scalar API docs at `/api/v1/docs`. That page loads `/api/v1/openapi.json` from the same controller process. Production does not register either backend runtime docs route.
