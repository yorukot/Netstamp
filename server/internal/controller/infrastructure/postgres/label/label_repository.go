...
func (r *LabelRepository) CreateLabel(ctx context.Context, input domainlabel.Label) (domainlabel.Label, error) {
...
func (r *LabelRepository) UpdateLabel(ctx context.Context, input domainlabel.Label) (domainlabel.Label, error) {
	ctx, span := postgres.StartDBSpan(ctx, pglabelTracer, "labels", "postgres.labels.update", "UPDATE", "UPDATE label")
	defer span.End()

	projectID, labelID, err := parseProjectAndLabelIDs(input.ProjectID, input.ID)
	if err != nil {
		return domainlabel.Label{}, err
	}
