...
func normalizeSeries(input string) ([]SeriesKey, error) {
	...
	if len(series) == 0 {
		return nil, resultshared.InvalidField("series", "must include at least one ping series", input)
	}
...
func normalizeSeriesKey(input string) (SeriesKey, error) {
	...
	case SeriesLatencyAvg:
		return SeriesLatencyAvg, nil
	case SeriesLatencyMin:
		return SeriesLatencyMin, nil
	case SeriesLatencyMax:
		return SeriesLatencyMax, nil
	case SeriesLossPercent:
		return SeriesLossPercent, nil
	default:
		return "", resultshared.InvalidField("series", "unsupported ping series", input)
	}
}
