...
- `internal/controller/application/auth/`, `internal/controller/application/systemsettings/`, `internal/controller/application/project/`, `internal/controller/application/label/`, `internal/controller/application/check/`, `internal/controller/application/probe/`, `internal/controller/application/result/{latest,ping,tcp,httpcheck,traceroute,shared}/`, `internal/controller/application/proberuntime/`, `internal/controller/application/{pingquery,tcpquery,httpquery}/`: controller use cases, ports, DTOs, errors, shared query policy, and feature orchestration. `systemsettings` owns the five Admin runtime resources and their cross-resource invariants; writes use internal per-resource transaction advisory locks, while `result/` itself is a thin compatibility facade for HTTP wiring.
...
- `internal/controller/infrastructure/`: PostgreSQL repositories and pool helpers, opaque session token management, Google and generic OIDC verification, GitHub OAuth profile/email exchange, Argon2id password hashing, and probe secret generation/verification.
...
No GraphQL, external message queues, payment, object-storage integrations, or standalone DNS probe executor are currently defined. The controller supports ping, TCP connect, HTTP/HTTPS, and traceroute check definitions, assignment payloads, runtime result ingestion, result queries, typed result persistence, alert incident evaluation, a DB-backed assignment refresh worker, and a notification outbox worker for webhook, Slack incoming webhook, Discord, Telegram, and SMTP email deliveries. The standalone probe agent executes all four check types through the shared scheduler, worker pool, result queue, and runtime result submitter.
...
Infrastructure settings come from environment variables or `.env`, with defaults and validation in `internal/controller/config/config.go`. Instance access policy, SMTP delivery, and OIDC/Google/GitHub providers are five Admin settings resources stored in PostgreSQL; provider and SMTP credentials are encrypted with `SYSTEM_SETTINGS_ENCRYPTION_KEY` and are never returned by Admin reads. Assignment refresh worker behavior is controlled by `ASSIGNMENT_REFRESH_WORKER_ENABLED`, `ASSIGNMENT_REFRESH_WORKER_INTERVAL`, `ASSIGNMENT_REFRESH_WORKER_BATCH_SIZE`, and `ASSIGNMENT_REFRESH_WORKER_STALE_TIMEOUT`. `WEB_DIR` is optional and should point at built frontend static files only in packaged/self-host deployments. Never commit real `.env` files, encryption or session hash keys, database passwords, trace endpoints with credentials, or production pseudonym keys.
...
Admin settings writes use internal per-resource transaction advisory locks for `access`, `smtp`, `auth.oidc`, `auth.google`, and `auth.github`; these locks are not exposed as client concurrency tokens. Acquire each lock inside the same transaction as the corresponding setting values and audit events. Access and SMTP mutations lock both resources in the fixed Access-then-SMTP order to enforce their shared email-delivery invariant without deadlocks. Data imports lock all five resources in the fixed `access`, `smtp`, `auth.oidc`, `auth.google`, `auth.github` order before replacing settings.
