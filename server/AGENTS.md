...
The observability compose setup requires `LOG_PSEUDONYM_KEY`, `DATABASE_PASSWORD`, `POSTGRES_EXPORTER_PASSWORD`, `AUTH_SESSION_HASH_KEY`, `SYSTEM_SETTINGS_ENCRYPTION_KEY`, and `GF_SECURITY_ADMIN_PASSWORD`. Passwords are hashed with Argon2id using `AUTH_ARGON2ID_*` settings. Browser sessions use opaque random tokens hashed with `AUTH_SESSION_HASH_KEY`; idle, absolute, touch, sudo, and external-flow timing are controlled by `AUTH_SESSION_IDLE_TTL`, `AUTH_SESSION_ABSOLUTE_TTL`, `AUTH_SESSION_TOUCH_INTERVAL`, `AUTH_SUDO_TTL`, and `AUTH_EXTERNAL_FLOW_TTL`. Google, GitHub, and generic OIDC are configured through encrypted Admin settings; `BACKEND_BASE_URL` remains the callback origin. Password reset links use only the validated `PUBLIC_WEB_BASE_URL`: non-local environments require it, while local environments without it report reset delivery as unavailable rather than trusting the request Host. Reset request throttling is controlled by `AUTH_PASSWORD_RESET_RATE_LIMIT_WINDOW`, `AUTH_PASSWORD_RESET_IP_LIMIT`, and `AUTH_PASSWORD_RESET_EMAIL_LIMIT`.
...
Current backend integrations are PostgreSQL/TimescaleDB, optional generic OpenID Connect authentication, optional OTLP trace export to VictoriaTraces, Prometheus-compatible metrics scraped by VictoriaMetrics, VictoriaLogs for Grafana log panels, outbound HTTPS alert notifications, and optional SMTP email delivery configured through Admin settings. Docker observability also includes Vector container log collection, PostgreSQL metrics via `postgres_exporter`, and a probe-agent dashboard for agent metrics when a probe exposes `NETSTAMP_PROBE_METRICS_ADDR`. The observability stack mounts `deployments/docker/postgres-exporter/queries.yaml` to backfill dashboard-specific activity-state metrics. No message queues, payment providers, or object storage clients are currently implemented.
