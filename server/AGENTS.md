...
- `internal/controller/transport/http/`: chi/Huma HTTP routing, auth, project, label, probe management, result query, probe runtime, system health routes, and middleware.
...
The database is PostgreSQL with TimescaleDB in Docker. The compose files currently default `TIMESCALEDB_IMAGE` to `timescale/timescaledb-ha:pg16` because the schema enables `timescaledb_toolkit` in addition to the core `timescaledb` extension. A Goose migration can only enable an extension that already exists inside the PostgreSQL image; `CREATE EXTENSION IF NOT EXISTS timescaledb_toolkit` does not install Toolkit into a lightweight image. If Toolkit usage is removed from migrations, the stack can use a smaller non-HA TimescaleDB image, but the image default and data volume path must be updated together.

The initial Goose migration enables `pgcrypto`, `citext`, `timescaledb`, and `timescaledb_toolkit`, then creates users, projects, project members, probes, selector-based ping checks, key/value labels, effective probe-check rows, ping results, and the ping results hypertable. A later migration adds a natural duplicate key for ping results on `(project_id, probe_id, check_id, started_at)`; the server/DB still generates result IDs. Traceroute schema is intentionally not present yet.
