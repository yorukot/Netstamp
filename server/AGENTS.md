...
- `internal/controller/infrastructure/`: PostgreSQL repositories and pool helpers, opaque session token management, Argon2id password hashing, and probe secret generation/verification.
...
Personal access tokens are stored as HMAC-SHA256 hashes in `api_tokens` using `AUTH_API_TOKEN_HASH_KEY`; plaintext values are returned only once. PAT requests do not use CSRF. Password changes/resets, administrator password resets, and account disablement revoke PATs, while normal session logout does not.
