...
The observability compose setup requires `LOG_PSEUDONYM_KEY`, `DATABASE_PASSWORD`, `POSTGRES_EXPORTER_PASSWORD`, `AUTH_SESSION_HASH_KEY`, and `GF_SECURITY_ADMIN_PASSWORD`. Passwords are hashed with Argon2id using `AUTH_ARGON2ID_*` settings. Browser sessions use opaque random tokens hashed with `AUTH_SESSION_HASH_KEY`; idle, absolute, touch, sudo, and external-flow timing are controlled by `AUTH_SESSION_IDLE_TTL`, `AUTH_SESSION_ABSOLUTE_TTL`, `AUTH_SESSION_TOUCH_INTERVAL`, `AUTH_SUDO_TTL`, and `AUTH_EXTERNAL_FLOW_TTL`. Google, GitHub, and generic OIDC are configured through encrypted Admin settings; `BACKEND_BASE_URL` remains the callback origin. Password reset links use `PUBLIC_WEB_BASE_URL` when set, otherwise the request origin, and reset request throttling is controlled by `AUTH_PASSWORD_RESET_RATE_LIMIT_WINDOW`, `AUTH_PASSWORD_RESET_IP_LIMIT`, and `AUTH_PASSWORD_RESET_EMAIL_LIMIT`.
