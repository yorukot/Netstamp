...
Infrastructure settings come from environment variables or `.env`, with defaults and validation in `internal/controller/config/config.go`. Instance access policy, SMTP delivery, and OIDC/Google/GitHub providers are five independently versioned Admin settings resources stored in PostgreSQL; provider and SMTP credentials are encrypted with `SYSTEM_SETTINGS_ENCRYPTION_KEY` and are never returned by Admin reads. Assignment refresh worker behavior is controlled by `ASSIGNMENT_REFRESH_WORKER_ENABLED`, `ASSIGNMENT_REFRESH_WORKER_INTERVAL`, `ASSIGNMENT_REFRESH_WORKER_BATCH_SIZE`, and `ASSIGNMENT_REFRESH_WORKER_STALE_TIMEOUT`. `WEB_DIR` is optional and should point at built frontend static files only in packaged/self-host deployments. Never commit real `.env` files, encryption or session hash keys, database passwords, trace endpoints with credentials, or production pseudonym keys.
...
Admin settings use `system_setting_revisions` rows as per-resource concurrency tokens. Mutations must lock the relevant revision row inside the same transaction as setting values and audit events; Access and SMTP mutations lock both resources in the fixed Access-then-SMTP order to enforce their shared email-delivery invariant without deadlocks.
