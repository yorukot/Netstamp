...
The initial Goose migration enables `pgcrypto`, `citext`, and `timescaledb`, then creates users, projects, project members, probes, selector-based ping checks, key/value labels, effective probe-check rows, ping results, and the ping results hypertable. Result series downsampling uses core TimescaleDB `time_bucket`; `timescaledb_toolkit` and DB-side `lttb` are intentionally not required. Probes and checks keep public UUIDs but also have internal bigint IDs, while `ping_results` stores bigint probe/check dimensions with `(probe_id, check_id, started_at)` as its primary key and no generated result ID. Traceroute schema is intentionally not present yet.
