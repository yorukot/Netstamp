...
- `internal/controller/application/auth/`, `internal/controller/application/systemsettings/`, `internal/controller/application/updatecheck/`, `internal/controller/application/project/`, `internal/controller/application/label/`, `internal/controller/application/check/`, `internal/controller/application/probe/`, `internal/controller/application/result/{latest,ping,tcp,httpcheck,traceroute,shared}/`, `internal/controller/application/proberuntime/`, `internal/controller/application/{pingquery,tcpquery,httpquery}/`: controller use cases, ports, DTOs, errors, shared query policy, and feature orchestration. `systemsettings` owns the six Admin runtime resources and their cross-resource invariants; writes use internal per-resource transaction advisory locks. `updatecheck` owns the in-memory release status cache and six-hour background worker, while `result/` itself is a thin compatibility facade for HTTP wiring.
- `internal/domain/identity/`, `internal/domain/project/`, `internal/domain/label/`, `internal/domain/check/`, `internal/domain/ping/`, `internal/domain/tcp/`, `internal/domain/httpcheck/`, `internal/domain/traceroute/`, and `internal/domain/probe/`: stable domain structs and domain-level sentinel errors.
...
- `internal/agent/config/`, `internal/agent/runtime/`, `internal/agent/scheduling/`, and `internal/agent/worker/`: probe agent environment loading, lifecycle orchestration, assignment scheduling, worker pool, result queue, and result submission. These packages use existing domain models for assignments, checks, ping/TCP/HTTP/traceroute config and results, and IP family values.
- `internal/agent/infrastructure/`: probe agent outbound integrations, including the controller runtime HTTP client, raw ICMP ping executor, TCP/HTTP/traceroute executors, and local network status discovery.
- `internal/controller/infrastructure/`: PostgreSQL repositories and pool helpers, opaque session token management, Google and generic OIDC verification, GitHub OAuth profile/email exchange, unauthenticated GitHub latest-release lookup, Argon2id password hashing, and probe secret generation/verification.
- `internal/controller/logger/`: controller zap logging helpers and application event recording. `internal/platform/normalize/`, `internal/platform/observability/`, and `internal/platform/version/` are shared helpers that do not depend on controller features.
...
The backend can serve a built frontend directory when `WEB_DIR` is set. In the self-host Docker image built by `deployments/Dockerfile`, this defaults to `/app/web`; `server/Dockerfile` remains backend-only, and local backend development leaves `WEB_DIR` unset so Vite owns the web app.
...
HTTP uses chi middleware and standard handlers under the fixed `/api/v1` prefix defined by `internal/platform/version.API`. The product version, API contract version, and minimum supported agent version are centralized in `internal/platform/version/version.go`; they are not environment settings. The published OpenAPI contract is authored in `api/` with TypeSpec, emitted to `docs/public/openapi.json`, and copied into `internal/controller/transport/http/openapi/openapi.json` for runtime serving. Scalar API docs are always exposed at `/api/v1/docs`, and the embedded schema is always exposed at `/api/v1/openapi.json`. API system routes are `/` and `/healthz` under the versioned API prefix; when `WEB_DIR` is set, top-level `/healthz` is also routed to API health, and non-API browser routes fall back to the React `index.html`. Install routes are `/install/agent.sh`, `/install/uninstall-agent.sh`, `/install/netstamp-agent-linux-amd64`, and `/install/netstamp-agent-linux-arm64`; they are unauthenticated and install only the agent binary, while probe ID and plaintext secret are supplied later to `netstamp-agent service install`. Auth routes include password register/login/reset, `/auth/methods`, provider-bound `/auth/external/{provider}/start` and callback flows for Google, GitHub, and generic OIDC, compatibility `/auth/oidc/*` routes, `/auth/sudo`, `/auth/me`, and `/auth/sessions`; `/auth/csrf`, `/auth/me`, `/auth/sessions`, `/users/me/*`, `/admin/*`, project routes, project label routes, project check routes, project probe creation, and project-scoped result routes are protected by the session auth middleware in `internal/controller/transport/http/middleware`. Probe runtime routes live under `/runtime/probes/{probe_id}/*` and use `Authorization: Probe <secret>` with the probe's one-time plaintext secret, not browser session auth.
...
`HTTP request -> chi route -> internal/controller/transport/http/proberuntime handler -> internal/controller/application/proberuntime.Service -> internal/controller/infrastructure/postgres/{probe,ping,tcp,httpcheck,traceroute} repositories -> sqlc.Queries -> PostgreSQL`

No GraphQL, external message queues, payment, object-storage integrations, or standalone DNS probe executor are currently defined. The controller supports ping, TCP connect, HTTP/HTTPS, and traceroute check definitions, assignment payloads, runtime result ingestion, result queries, typed result persistence, alert incident evaluation, a DB-backed assignment refresh worker, a notification outbox worker for webhook, Slack incoming webhook, Discord, Telegram, and SMTP email deliveries, and an opt-in six-hour GitHub release update check. The standalone probe agent executes all four check types through the shared scheduler, worker pool, result queue, and runtime result submitter.
...
- Application (`internal/controller/application/*`): business orchestration, service methods, ports, app errors, feature event semantics, and use-case spans. Depend on interfaces, not concrete pgx, HTTP framework, or token implementation details.
...
- Infrastructure (`internal/controller/infrastructure/postgres`, `internal/controller/infrastructure/security`): pgx/sqlc persistence, database error translation, opaque session token hashing/generation, and Argon2id password hashing. Infrastructure packages should import domain packages for models and sentinel errors, but should not import application packages or depend on application DTOs.
...
- Cross-cutting (`internal/controller/logger`, `internal/platform/observability`, `internal/platform/version`): request-scoped loggers, auth event recording, trace fields, tracer provider setup, span helpers, and shared product/API version constants.
...
- Command features: `auth`, `systemsettings`, `project`, `label`, `check`, `probe`, `proberuntime`, `assignment`, `user`, `alert`, and the mutating public status page/element use cases in `publicstatus`.
...
The backend uses authenticated users plus two separate authorization planes: instance-level system administrators in `system_user_roles`, and project-scoped membership roles in `project_members`. System administrators can manage instance settings and other system administrators, but system admin status does not grant project membership or project permissions. HTTP auth middleware proves identity only; application services own authorization.

Session-only chi routes use `internal/controller/transport/http/middleware.RequireAuth`. Project resource routes use `RequireUserAuth` and accept either the configured HTTP-only session cookie or `Authorization: Bearer <personal-access-token>`. Bearer scopes are enforced in transport and existing application services still enforce the token owner's current project role. Account, member, invite, token-management, and administrator routes remain session-only. The middleware never falls back to a cookie when an Authorization header is present. Local development uses `netstamp_session`; non-local environments use `__Host-netstamp_session` with `Secure`, `HttpOnly`, `SameSite=Strict`, `Path=/`, and no `Domain`. Password and external-provider login create a server-side `auth_sessions` row with authentication method, recent-authentication metadata, sudo eligibility, request User-Agent, and optional external identity. `/auth/logout` revokes the current server-side row and clears the cookie. `/auth/sessions` lists the current user's active sessions, `DELETE /auth/sessions` revokes every owned session including the current session and clears its cookie, and `DELETE /auth/sessions/{session_id}` revokes one owned session; revoking the current session also clears its cookie. Transport handlers read the current user id through middleware context helpers and pass it into application service inputs. Keep role checks out of HTTP handlers except for translating application errors into HTTP responses.

Passwords are optional credentials stored in `password_credentials`, not nullable columns on `users`. External identities are stored in `user_identities` and are keyed by provider, issuer, and subject. Matching email addresses never auto-link an external identity to an existing user; linking is an explicit, authenticated account action. Password or identity removal must retain at least one login method, with the final guard enforced atomically by SQL. Sensitive account, personal-access-token, settings, and administrator mutations use `RequireSudo`; password and linked Google, GitHub, or generic OIDC reauthentication refresh the same session for the configured `AUTH_SUDO_TTL` (five minutes by default). External reauthentication validates provider `auth_time` when available and otherwise uses successful completion of a new, short-lived OAuth flow bound to the current Netstamp session.

State-changing browser session routes are protected by CSRF middleware. The frontend obtains a per-session synchronizer token from `/auth/csrf` and sends it as `X-CSRF-Token`; the backend stores only the token hash in `auth_sessions`. Keep unauthenticated auth flows and probe runtime routes outside session CSRF enforcement unless they begin using browser session cookies.

Personal access tokens are stored as HMAC-SHA256 hashes in `api_tokens` using `AUTH_API_TOKEN_HASH_KEY`; plaintext values are returned only once. PAT requests do not use CSRF. Password changes, resets, and removals, administrator password changes, and account disablement revoke PATs, while normal session logout does not.

System admin rows are stored in `system_user_roles` with role `admin`. Registration bootstraps the first system admin only when no admin exists yet; later system admin grants and revocations go through the admin application service, require an existing system admin caller, prevent self-removal, and prevent removing the final system admin.
...
- Orchestrators and workers must not hide technical errors behind caller events. Alert evaluation errors go through `logger.AlertEvalEventRecorder`, while assignment refresh, notification outbox, and update-check workers log swallowed `RunOnce` errors with `worker.name`, `worker.operation`, and `error` fields before continuing.
...
- Never log raw passwords, password hashes, session tokens, CSRF tokens, session hash keys, cookies, database passwords, probe plaintext secrets, probe secret hashes, raw request bodies, or raw personal data.
...
Commands below come from the root `Justfile`, root `package.json`, `server/.air.toml`, `server/Dockerfile`, the self-host files under `deployments/`, and the development observability stack under `deployments/dev/`.
...
- `just api-openapi` or `pnpm generate:openapi`: write the TypeSpec OpenAPI schema to `docs/public/openapi.json`, copy it to the backend embedded OpenAPI artifact, and regenerate the web API types and API metadata.
...
- `docker compose -f deployments/dev/compose.yaml up --build`: build and run the development observability stack with PostgreSQL, VictoriaTraces, VictoriaMetrics, VictoriaLogs, Vector, Grafana, nginx, controller, migrations, and the backend image's agent install artifacts.
...
When an application error needs to tell the client exactly which request field is invalid, wrap the sentinel error with `internal/controller/application/validation.New` or `NewFields`. The wrapped error must still satisfy `errors.Is(err, ErrInvalidInput)` through `Unwrap`, while carrying `FieldError` metadata: `Field`, `Code`, `Message`, and `Value`. Transport handlers should extract this metadata with `appvalidation.FieldErrors(err)` and convert it to `httpx.ErrorDetail` values with HTTP locations. Feature handlers follow this pattern in their `context.go`, mapping fields to locations such as `body.name`, `body.selector`, `body.labelIds`, `path.ref`, or `body` for whole-body validation failures like an empty patch.
...
Infrastructure settings come from environment variables or `.env`, with defaults and validation in `internal/controller/config/config.go`. Instance access policy, update checking, SMTP delivery, and OIDC/Google/GitHub providers are six Admin settings resources stored in PostgreSQL; the update-check row is created lazily only after an administrator changes its default-enabled value. Provider and SMTP credentials are encrypted with `SYSTEM_SETTINGS_ENCRYPTION_KEY` and are never returned by Admin reads. The update worker uses a fixed six-hour interval, starts with an immediate run, and calls the public `yorukot/netstamp` latest-release endpoint without a token. Assignment refresh worker behavior is controlled by `ASSIGNMENT_REFRESH_WORKER_ENABLED`, `ASSIGNMENT_REFRESH_WORKER_INTERVAL`, `ASSIGNMENT_REFRESH_WORKER_BATCH_SIZE`, and `ASSIGNMENT_REFRESH_WORKER_STALE_TIMEOUT`. `WEB_DIR` is optional and should point at built frontend static files only in packaged/self-host deployments. Never commit real `.env` files, encryption or session hash keys, database passwords, trace endpoints with credentials, or production pseudonym keys.

The development observability compose setup requires `LOG_PSEUDONYM_KEY`, `DATABASE_PASSWORD`, `POSTGRES_EXPORTER_PASSWORD`, `AUTH_SESSION_HASH_KEY`, `AUTH_API_TOKEN_HASH_KEY`, `SYSTEM_SETTINGS_ENCRYPTION_KEY`, and `GF_SECURITY_ADMIN_PASSWORD`. Passwords are hashed with Argon2id using `AUTH_ARGON2ID_*` settings. Browser sessions use opaque random tokens hashed with `AUTH_SESSION_HASH_KEY`; idle, absolute, touch, sudo, and external-flow timing are controlled by `AUTH_SESSION_IDLE_TTL`, `AUTH_SESSION_ABSOLUTE_TTL`, `AUTH_SESSION_TOUCH_INTERVAL`, `AUTH_SUDO_TTL`, and `AUTH_EXTERNAL_FLOW_TTL`. Google, GitHub, and generic OIDC are configured through encrypted Admin settings. `PUBLIC_BASE_URL` is the single callback and user-facing origin; non-local environments require it, production requires HTTPS, and local environments may leave it empty. Reset request throttling is controlled by `AUTH_PASSWORD_RESET_RATE_LIMIT_WINDOW`, `AUTH_PASSWORD_RESET_IP_LIMIT`, and `AUTH_PASSWORD_RESET_EMAIL_LIMIT`.

## Database & Persistence

The database is PostgreSQL with TimescaleDB in Docker. The compose files default `TIMESCALEDB_IMAGE` to the tested lightweight `timescale/timescaledb:2.20.3-pg16` image and mount data at `/var/lib/postgresql/data`. Do not switch back to the heavier `timescale/timescaledb-ha` image unless the schema needs extensions or services that are not present in the lightweight image.

The v0.1.0 schema is created by the single fresh-install Goose baseline `00001_v0_1_0.sql`, which enables `pgcrypto`, `citext`, and `timescaledb` and creates the complete release schema. It replaces all pre-release migration history: development databases and PostgreSQL volumes that ran the old chain must be backed up if needed and recreated before v0.1.0. The baseline is intentionally irreversible; `migrate down` returns an error without removing the schema or version record. After v0.1.0, released migration files are append-only and immutable.

Current Ping long-term queries use `ping_result_rollups_1m` for safely re-aggregated result count, success count, sent/received count, average RTT sum/count, min RTT, and max RTT data. Obsolete pre-release Ping/TCP/traceroute rollups, Ping sample-density storage, traceroute topology observations, and the system-setting revision table/rollback sequence are omitted from the baseline. Raw result hypertables and traceroute hop hypertables use 1-day chunk intervals with 3-day retention. Traceroute also keeps permanent 1-minute sampled run snapshots in `traceroute_sampled_runs_1m`; a Timescale user-defined job refreshes recent buckets by selecting the latest run per probe/check/bucket and storing ordered hop JSON; its hop join is bounded to the refresh window so TimescaleDB chunk exclusion applies, and its upsert only rewrites a bucket when a strictly newer run exists. Result series downsampling uses core TimescaleDB `time_bucket`; `timescaledb_toolkit` and DB-side `lttb` are intentionally not required. Probes and checks keep public UUIDs but also have internal bigint IDs, while `ping_results`, `tcp_results`, and `traceroute_results` store bigint probe/check dimensions with `(probe_id, check_id, started_at)` as their primary key and no generated result ID. Traceroute hop rows store per-hop aggregate RTT fields and `rtt_samples_ms` arrays with `(probe_id, check_id, started_at, hop_index)` as their primary key. Because TimescaleDB does not support foreign keys from one hypertable to another, `traceroute_result_hops` does not keep a database foreign key to `traceroute_results`; repository writes create the run and hop rows in the same transaction, and query paths select runs first before joining hops.

TimescaleDB 2.20.3 or newer is required for Netstamp's Hypercore columnstore configuration on continuous aggregates. Raw Ping, TCP, HTTP, traceroute, and traceroute-hop chunks become eligible for columnstore conversion after 1 day and are checked every 6 hours. Permanent 1-minute result rollups and traceroute sampled runs become eligible after 7 days and are checked every 12 hours so aggregate refreshes remain in rowstore. All layouts segment by `(probe_id, check_id)` and order by descending time; policy first runs are staggered to avoid concurrent backfill load. Removing a columnstore policy stops future conversion without automatically rewriting existing columnstore chunks back to rowstore.

Admin settings writes use internal per-resource transaction advisory locks for `access`, `smtp`, `auth.oidc`, `auth.google`, and `auth.github`; these locks are not exposed as client concurrency tokens. Acquire each lock inside the same transaction as the corresponding setting values and audit events. Access and SMTP mutations lock both resources in the fixed Access-then-SMTP order to enforce their shared email-delivery invariant without deadlocks.

Add schema changes as new timestamped Goose migrations under `db/migrations/`, following `db/migrations/README.md`. New versions must be greater than the retired pre-release maximum `202607300001`; never edit, delete, rename, or renumber a released migration. Add typed SQL queries under `db/query/*.sql`, then run `just backend-sqlc`. Do not edit `internal/controller/infrastructure/postgres/sqlc/*.go` manually. Keep repositories responsible for mapping sqlc rows and pgx errors into domain/application types.

## External Integrations

Current backend integrations are PostgreSQL/TimescaleDB, optional generic OpenID Connect authentication, optional OTLP trace export to VictoriaTraces, Prometheus-compatible metrics scraped by VictoriaMetrics, VictoriaLogs for Grafana log panels, outbound HTTPS alert notifications, and optional SMTP email delivery configured through Admin settings. Docker observability also includes Vector container log collection, PostgreSQL metrics via `postgres_exporter`, and a probe-agent dashboard for agent metrics when a probe exposes `NETSTAMP_PROBE_METRICS_ADDR`. The development observability stack mounts `deployments/dev/postgres-exporter/queries.yaml` to backfill dashboard-specific activity-state metrics. No message queues, payment providers, or object storage clients are currently implemented.
