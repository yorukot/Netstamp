...
- `internal/controller/infrastructure/`: PostgreSQL repositories and pool helpers, opaque session token management, generic OIDC discovery/code exchange, Argon2id password hashing, and probe secret generation/verification.
...
Session-only chi routes use `internal/controller/transport/http/middleware.RequireAuth`. Project resource routes use `RequireUserAuth` and accept either the configured HTTP-only session cookie or `Authorization: Bearer <personal-access-token>`. Bearer scopes are enforced in transport and existing application services still enforce the token owner's current project role. Account, member, invite, token-management, and administrator routes remain session-only. The middleware never falls back to a cookie when an Authorization header is present. Local development uses `netstamp_session`; non-local environments use `__Host-netstamp_session` with `Secure`, `HttpOnly`, `SameSite=Strict`, `Path=/`, and no `Domain`. Password and OIDC login create a server-side `auth_sessions` row with authentication method, recent-authentication time, request User-Agent, and optional external identity. `/auth/logout` revokes the current server-side row and clears the cookie. `/auth/sessions` lists the current user's active sessions, `DELETE /auth/sessions` revokes every owned session including the current session and clears its cookie, and `DELETE /auth/sessions/{session_id}` revokes one owned session; revoking the current session also clears its cookie. Transport handlers read the current user id through middleware context helpers and pass it into application service inputs. Keep role checks out of HTTP handlers except for translating application errors into HTTP responses.

Passwords are optional credentials stored in `password_credentials`, not nullable columns on `users`. External identities are stored in `user_identities` and are keyed by issuer plus subject. Matching email addresses never auto-link an OIDC identity to an existing user; linking is an explicit, authenticated account action. Password or identity removal must retain at least one login method, with the final guard enforced atomically by SQL. Sensitive account, personal-access-token, backup, settings, and administrator mutations use `RequireSudo`; a password or forced OIDC reauthentication refreshes the session's recent-authentication timestamp for the configured `AUTH_SUDO_TTL` (five minutes by default).
